...
var RemoveItPermanently = {
    ...
    load: function() {
        try {
            ...
            catch (e) {   // Some windows (ie. XUL dialogs) don't have a getBrowser so we just ignore these windows
                ;
            }
        ...
        if (framemenuitem) {
            ...
            if (iFrame) {
                ...
                if (frameNode.nodeName == "FRAME") {
                    // Deleting this might break the structure of ancestor elements, so hide it instead
    ...
    getAllDomain: function(doc) {
        var domainName = doc.domain;   // Assume this is the domain unless there are more than one '.' between the other elements.   Must figure out a better way to do this!!!
...
var ripListener = {
    QueryInterface: function(aIID) {
        if (aIID.equals(Components.interfaces.nsIWebProgressListener) || aIID.equals(Components.interfaces.nsISupportsWeakReference) || aIID.equals(Components.interfaces.nsIXULBrowserWindow) || aIID.equals(Components.interfaces.nsISupports)) {
            return this;
        }
